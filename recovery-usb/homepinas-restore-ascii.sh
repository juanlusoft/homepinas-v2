...
BACKTITLE="[HOME] HomePiNAS Recovery System v1.0"
...
# Show info dialog
show_info() {
    dialog --backtitle "$BACKTITLE" --title "[INFO]  Información" --msgbox "$1" 8 60
}
...
setup_network() {
    dialog --backtitle "$BACKTITLE" --title "[NET] Red" \
    ...
    dialog --backtitle "$BACKTITLE" --title "[WARN] Red" --yesno \
...
discover_nas_tui() {
    ...
    dialog --backtitle "$BACKTITLE" --title "[SEARCH] Buscando NAS" \
    ...
    dialog --backtitle "$BACKTITLE" --title "[WARN] NAS no encontrado" --yesno \
...
select_target_disk() {
    ...
    local selected_disk
    selected_disk=$(dialog --backtitle "$BACKTITLE" --title "[SAVE] Seleccionar disco destino" \
        --menu "[WARN]  EL DISCO SELECCIONADO SERÁ BORRADO COMPLETAMENTE\n\nElige el disco destino:" 18 65 8 \
        "${menu_items[@]}" 3>&1 1>&2 2>&3)
    ...
    dialog --backtitle "$BACKTITLE" --title "[WARN]  ¡ATENCIÓN!" --yesno \
        "¡TODOS LOS DATOS en ${selected_disk} se PERDERÁN!\n\nDisco: ${selected_disk}\nInfo: ${disk_info}\n\n¿Estás SEGURO de que quieres continuar?" 12 60
    
    [ $? -ne 0 ] && return 1

    # Triple confirm for safety
    dialog --backtitle "$BACKTITLE" --title "[WARN]  ÚLTIMA CONFIRMACIÓN" --yesno \
...
restore_single_image() {
    ...
    (
        ...
        echo "100"
    ) | dialog --backtitle "$BACKTITLE" --title "[SAVE] Restaurando imagen" \
...
restore_partition_images() {
    ...
    (
        ...
        echo "100"
    ) | dialog --backtitle "$BACKTITLE" --title "[SAVE] Restaurando particiones" \
        --gauge "Preparando..." 8 60 0
...
disk_utilities_menu() {
    while true; do
        ...
        choice=$(dialog --backtitle "$BACKTITLE" --title "🔧 Utilidades de disco" \
            ...
            "info" "[BACKUP] Información de discos" \
            "smart" "[SEARCH] Estado SMART" \
...
show_system_info() {
    ...
    dialog --backtitle "$BACKTITLE" --title "[INFO]  Información del sistema" \
        --msgbox "$info" 25 70
}
...
main_menu() {
    while true; do
        local nas_status="[ERROR] Sin conexión"
        ...
        choice=$(dialog --backtitle "$BACKTITLE" --title "Menú principal -- NAS: ${nas_status}" \
            ...
            "info" "[INFO]  Información del sistema" \
...
main() {
    ...
    # Welcome screen
    dialog --backtitle "$BACKTITLE" --title "[HOME] HomePiNAS Recovery" --msgbox \
        "Bienvenido al sistema de recuperación HomePiNAS\n\n\
Este asistente te guiará para:\n\n\
  [SEARCH] Encontrar tu NAS en la red\n\
  📦 Seleccionar un backup existente\n\
  [SAVE] Restaurar en el disco destino\n\n\
Soporta:\n\
  • Linux (dd, partclone)\n\
  • Windows (WIM, ntfsclone)\n\
  • Restauración de archivos individuales\n\n\
Asegúrate de que:\n\
  [OK] El NAS está encendido y en la misma red\n\
  [OK] El disco destino está conectado\n\
  [OK] Tienes credenciales del NAS" 22 58
